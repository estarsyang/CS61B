...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        /** TODO: Your code here. */
        // child > its parent or child is the root
        if (index == 1){
            return;
        }
    ...
    private void sink(int index) {
        ...
        // complete tree
        int smallIdx = min(leftIdx, rightIdx);
        if (min(index, smallIdx) == smallIdx) {
            swap(index, smallIdx);
            sink(smallIdx);
        }
        return;
    ...
    public void changePriority(T item, double priority) {
        ...
        Node curNode = getNode(itemIndex);
